...
public class ParkingLot extends ComponentDefinition {
	...
	public static void processRequests(
		...
		System.out.printf(
				"Processing request issued by %s on parking from %d to %d.\n",
		...
		for (ParkingPlaceId place : parkingPlaces) {
			...
			// if no collision with the scheduled items is found, then the
			// request
			...
			if (canBeScheduled(incomingRequest.scheduleItem, placeSchedule)) {
				...
				processedResponses.put(incomingRequest.requestId, new Response(
						incomingRequest));
	...
	private static boolean canBeScheduled(ParkingLotScheduleItem item,
			List<ParkingLotScheduleItem> schedule) {
