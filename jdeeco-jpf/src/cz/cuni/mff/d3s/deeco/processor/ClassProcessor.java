...
public class ClassProcessor {
	 ...
	 */
	public static RuntimeMetadataProvider processClasses(
	 ...
	 * @throws MalformedURLException
	 */
	public static RuntimeMetadataProvider processClasses(List<String> classes,
			List<URL> dirURLs, ClassLoader parentClassLoader) {
		InstanceRuntimeMetadataProvider result = null;
		if (classes.size() > 0) {
			// dirURLs.addAll(Arrays.asList(((URLClassLoader)ClassLoader.getSystemClassLoader()).getURLs()));
			result = new InstanceRuntimeMetadataProvider();
			...
			for (String cs : classes) {
				try {
					...
					if (isComponentDefinition(clazz)) {
						result.fromComponentInstance(extractInitialKnowledge(clazz));
					} else if (isEnsembleDefinition(clazz)) {
						result.fromEnsembleDefinition(clazz);
					}
				} catch (Exception e) {
