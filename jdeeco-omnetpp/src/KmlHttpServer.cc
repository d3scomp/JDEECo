...
void KmlHttpServer::initialize()
{
    ...
    socketDataProvider = check_and_cast<cSocketDataProvider *>(simulation.getScheduler());
    socketDataProvider->setInterfaceModule(this, rtEvent, recvBuffer, 4000, &numRecvBytes);
}
...
void KmlHttpServer::handleSocketEvent()
{
    ...
    // send back reply
    socketDataProvider->sendBytes(reply.c_str(), reply.size());
    socketDataProvider->close();
}
...
const char *INDEX_HTML =
    ...
    "\n"
    "         setTimeout(\"networkLink.setFlyToView(false)\", 15000);\n"
    "      }\n"
    "\n"
