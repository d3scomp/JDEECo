...
public class ConvoyRebroadcastTest {
	 ...
	 * 
	 */
	@Test @Ignore("Please use this only to verify performance")
	public void rebroadcastingPerformanceTest() throws Exception {
		// Create main application container
		SimulationTimer simulationTimer = new DiscreteEventTimer(); // also "new WallTimeSchedulerNotifier()"
		DEECoSimulation realm = new DEECoSimulation(simulationTimer);
		realm.addPlugin(new SimpleBroadcastDevice(25, 5, 250, 8192));
		realm.addPlugin(Network.class);
		realm.addPlugin(KnowledgeInsertingStrategy.class);
		realm.addPlugin(RebroadcastStrategy.class);
		realm.addPlugin(DefaultKnowledgePublisher.class);
		...
		for(int i = 0; i < NODES; ++i) {
			// Create DEECo node 1 - leader
			DEECoNode deeco = realm.createNode(new PositionPlugin(rand.nextInt(PLAYGROUND_WIDTH), rand.nextInt(PLAYGROUND_HEIGHT)));
			
			if(rand.nextBoolean()) {
				// Deploy components and ensembles
				deeco.deployComponent(new Leader("L" + i, System.out));
			} else {
				// Deploy components and ensembles
				deeco.deployComponent(new Follower("F" + i, System.out));
			}
