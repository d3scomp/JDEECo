...
import cz.cuni.mff.d3s.deeco.knowledge.KnowledgePathUtils;
...
/**
 * A helper class which constructs a sample RuntimeModel to be used in testing.
 * For convenience, it references the key entities of the model by public fields
 * of the class. Note that the model does not contain the knowledge managers
 * associated with the component instance. They have to be supplied from
 * outside.
 * 
...
public class SampleRuntimeModel {
	...
	public Parameter exchangeParamCoordIn, exchangeParamCoordOut,
			exchangeParamCoordInOut;
	public Parameter exchangeParamMemberIn, exchangeParamMemberOut,
			exchangeParamMemberInOut;
	...
	public static void processMethod(Integer in, ParamHolder<Integer> out,
			ParamHolder<Integer> inOut) {
	...
	public static void exchangeMethod(Integer coordIn,
			ParamHolder<Integer> coordOut, ParamHolder<Integer> coordInOut,
			Integer memberIn, ParamHolder<Integer> memberOut,
			ParamHolder<Integer> memberInOut) {
	...
	public void setOtherKnowledgeManagersAccess(
			KnowledgeManagersView knowledgeManagersView) {
		componentInstance
				.setOtherKnowledgeManagersAccess(knowledgeManagersView);
	}

	private Parameter createParameter(ParameterDirection direction,
	...
	public SampleRuntimeModel() throws Exception {
		...
		process.setMethod(SampleRuntimeModel.class.getMethod("processMethod",
				Integer.class, ParamHolder.class, ParamHolder.class));
		// process.setComponentInstance(componentInstance);
		processParamIn = createParameter(ParameterDirection.IN, "level1", "in");
		processParamOut = createParameter(ParameterDirection.OUT, "level1",
				"out");
		processParamInOut = createParameter(ParameterDirection.INOUT, "level1",
				"inout");
		...
		processSchedulingSpec.setPeriod(10); // FIXME TB: what does this number
												// mean?
		...
		ensembleSchedulingSpec.setPeriod(10); // FIXME TB: what does this number
												// mean?
		...
		membershipCondition.setMethod(SampleRuntimeModel.class.getMethod(
				"membershipMethod", Integer.class, Integer.class));
		membershipParamCoord = createParameter(ParameterDirection.IN, "<C>",
				"level1", "out");
		membershipParamMember = createParameter(ParameterDirection.IN, "<M>",
				"level1", "out");
		...
		knowledgeExchange.setMethod(SampleRuntimeModel.class.getMethod(
				"exchangeMethod", Integer.class, ParamHolder.class,
				ParamHolder.class, Integer.class, ParamHolder.class,
				ParamHolder.class));
		exchangeParamCoordIn = createParameter(ParameterDirection.IN, "<C>",
				"level1", "out");
		exchangeParamCoordOut = createParameter(ParameterDirection.OUT, "<C>",
				"level1", "trigger");
		exchangeParamCoordInOut = createParameter(ParameterDirection.INOUT,
				"<C>", "level1", "in");
		exchangeParamMemberIn = createParameter(ParameterDirection.IN, "<M>",
				"level1", "out");
		exchangeParamMemberOut = createParameter(ParameterDirection.OUT, "<M>",
				"level1", "trigger");
		exchangeParamMemberInOut = createParameter(ParameterDirection.INOUT,
				"<M>", "level1", "in");
