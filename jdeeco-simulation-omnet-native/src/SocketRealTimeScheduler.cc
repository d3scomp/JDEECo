...
#include "SocketRealTimeScheduler.h"
...
Register_Class(cSocketRealTimeScheduler);

Register_GlobalConfigOption(CFGID_SOCKETREALTIMESCHEDULER_PORT, "socketrealtimescheduler-port", CFG_INT, "4242", "When cSocketRealTimeScheduler is selected as scheduler class: the port number cSocketRTScheduler listens on.");
...
cSocketRealTimeScheduler::cSocketRealTimeScheduler() : cScheduler()
{
    listenerSocket = INVALID_SOCKET;
    connSocket = INVALID_SOCKET;
}

cSocketRealTimeScheduler::~cSocketRealTimeScheduler()
{
}

void cSocketRealTimeScheduler::startRun()
{
    ...
    port = ev.getConfig()->getAsInt(CFGID_SOCKETREALTIMESCHEDULER_PORT);
    setupListener();
}

void cSocketRealTimeScheduler::setupListener()
{
...
void cSocketRealTimeScheduler::endRun()
{
}

void cSocketRealTimeScheduler::executionResumed()
{
    gettimeofday(&baseTime, NULL);
    baseTime = timeval_substract(baseTime, SIMTIME_DBL(simTime()));
}

void cSocketRealTimeScheduler::setInterfaceModule(cModule *mod, cMessage *notifMsg, char *buf, int bufSize, int *nBytesPtr)
{
    if (module)
...
bool cSocketRealTimeScheduler::receiveWithTimeout(long usec)
{
...
int cSocketRealTimeScheduler::receiveUntil(const timeval& targetTime)
{
...
void cSocketRealTimeScheduler::sendBytes(const char *buf, size_t numBytes)
{
    if (connSocket==INVALID_SOCKET)
...
void cSocketRealTimeScheduler::close()
{
    if (connSocket!=INVALID_SOCKET)
    {
        closesocket(connSocket);
