...
JNIEXPORT void JNICALL _Java_cz_cuni_mff_d3s_deeco_simulation_Simulation_nativeRegister(JNIEnv *env, jobject jsimulation, jobject object, jstring id) {
	...
	if (JNI_OK == status) {
		jDEECoRuntimes.push_back(new jDEECoRuntime(env->NewGlobalRef(object), jvm, cstring));
...
JNIEXPORT void JNICALL _Java_cz_cuni_mff_d3s_deeco_simulation_Simulation_nativeSendPacket(JNIEnv *env, jobject jsimulation, jstring id, jbyteArray packet, jstring recipient) {
	const char * cstring = env->GetStringUTFChars(id,0);
...
JNIEXPORT void JNICALL _Java_cz_cuni_mff_d3s_deeco_simulation_Simulation_nativeRun(JNIEnv *env, jobject jsimulation, jstring environment) {
...
DLLEXPORT_OR_IMPORT void jDEECoModule::jDEECoOnHandleMessage(cMessage *msg) {
	if (jvm != NULL && host != NULL) {
		...
		if (opp_strcmp(msg->getName(), JDEECO_TIMER_MESSAGE) == 0) {
			if (simTime().dbl() == currentCallAtTime) {
			...
			env->SetByteArrayRegion(jArray, 0, jPacket->getDataArraySize(), (jbyte *) buffer);
