...
void JDEECoPacket::copy(const JDEECoPacket& other)
{
    ...
    this->data_var = (other.data_arraysize==0) ? NULL : new signed char[other.data_arraysize];
...
void JDEECoPacket::parsimUnpack(cCommBuffer *b)
{
    ...
    if (data_arraysize==0) {
        ...
        this->data_var = new signed char[data_arraysize];
...
{
    signed char *data_var2 = (size==0) ? NULL : new signed char[size];
...
signed char JDEECoPacket::getData(unsigned int k) const
{
    if (k>=data_arraysize) throw cRuntimeError("Array of size %d indexed by %d", data_arraysize, k);
    return data_var[k];
}

void JDEECoPacket::setData(unsigned int k, signed char data)
{
    if (k>=data_arraysize) throw cRuntimeError("Array of size %d indexed by %d", data_arraysize, k);
